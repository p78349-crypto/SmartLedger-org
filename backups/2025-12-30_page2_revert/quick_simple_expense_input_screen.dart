...
class _QuickSimpleExpenseInputScreenState
  ...
  final List<({String description, double amount, String payment, String store})> _displayedItems = [];
  ...
  Future<void> _openHistory() async {
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => _QuickSimpleExpenseHistoryScreen(
  ...
  ({
    ...
    if (amountMatch != null) {
      ...
      rawAmount = text.substring(amountMatch.start, amountMatch.end)
          .replaceAll(',', '');
    ...
    final tokens =
        text.split(' ').where((t) => t.trim().isNotEmpty).toList();
  ...
  Future<void> _goNext() async {
    ...
    if (parsed == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            '금액이 필요합니다. 예: 커피 1잔 3000원 버거킹 (결제/매장은 선택)',
          ),
        ),
      );
      return;
  ...
  Widget build(BuildContext context) {
    const bottomActionsHeightPad = 128.0;
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(
                ...
                labelText: '내용',
                hintText: '예: 커피 1잔 3000원 현대카드 메가커피',
              ),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: Padding(
                      ...
                      child: Text(
                        '${item.description} · ${CurrencyFormatter.format(item.amount)}${item.payment != '미지정' ? ' · ${item.payment}' : ''}${item.store != '미지정' ? ' · ${item.store}' : ''}',
                        maxLines: 3,
                        overflow: TextOverflow.ellipsis,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<QuickSimpleExpenseInputEntry>>(
        future: QuickSimpleExpenseInputHistoryService().loadEntries(accountName),
