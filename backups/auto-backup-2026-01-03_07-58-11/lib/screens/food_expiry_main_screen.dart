...
class _RecipePickerDialogState extends State<_RecipePickerDialog> {
  ...
  final List<String> _cuisines = ['All', 'Korean', 'Western', 'Japanese', 'Chinese', 'Other'];
                        ...
                        return ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () async {
                              await showDialog(
                                context: context,
                                builder: (c) => _RecipeUpsertDialog(existing: r),
                              );
                              setState(() {});
                            },
                          ),
...
class _RecipeUpsertDialogState extends State<_RecipeUpsertDialog> {
  ...
  final List<String> _cuisines = ['Korean', 'Western', 'Japanese', 'Chinese', 'Other'];
  ...
  Future<void> _importFromHistory() async {
    ...
    final history = await UserPrefService.getShoppingCartHistory(accountName: accountName);
    if (!mounted) return;
                ...
                if (widget.existing != null)
                  TextButton(
                    onPressed: () async {
                      ...
                      if (confirm == true) {
                        await RecipeService.instance.deleteRecipe(widget.existing!.id);
...
class _IngredientUpsertDialog extends StatefulWidget {
  ...
  @override
  State<_IngredientUpsertDialog> createState() => _IngredientUpsertDialogState();
}
        ...
        FilledButton(
          onPressed: () {
            ...
            if (name.isNotEmpty && qty > 0) {
              widget.onAdd(RecipeIngredient(name: name, quantity: qty, unit: unit));
              Navigator.pop(context);
            }
          },
...
class _FoodExpiryUpsertDialogState extends State<_FoodExpiryUpsertDialog> {
  ...
  void _updateTotal() {
    ...
    // Only update if different to avoid cursor jumping if we were editing it directly (though we might disable direct edit if using this mode)
                        ...
                        children: [
                          ...items.map((item) {
                            return ListTile(
                              title: Text(item.name),
                              subtitle: Text(
                                '${item.quantity}개 / ${DateFormat('HH:mm').format(item.at)}',
                              ),
                              trailing: IconButton(
                                icon: const Icon(Icons.playlist_add),
                                onPressed: () {
                                  Navigator.of(ctx).pop();
                                  _startImport(items, startIndex: items.indexOf(item));
                                },
                              ),
                            );
                          }),
                          ListTile(
                            title: const Text('이 날짜의 모든 항목 가져오기',
  ...
  void _startImport(List<ShoppingCartHistoryEntry> items, {int startIndex = 0}) {
    ...
    if (_importQueue.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('모든 항목을 가져왔습니다.')),
        );
      }
      return;
    }
  ...
  Future<void> _save() async {
    ...
    final unit = _unitController.text.trim().isEmpty ? '개' : _unitController.text.trim();
  ...
  Widget build(BuildContext context) {
    ...
    final currentImportIndex = _importTotal - remaining; // 1-based index for display? No, 0 items done.
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    if (isImporting)
                      Chip(
                        label: Text('${currentImportIndex + 1} / $_importTotal'),
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(
                          _pickedExpiryDate == null
                              ? (p == null
                                  ? '날짜 선택'
                                  : '예측: ${DateFormat('yyyy-MM-dd').format(p.suggestedExpiryDate)}')
                              : '수동: ${DateFormat('yyyy-MM-dd').format(_pickedExpiryDate!)}',
                        ),
                      ),
                    ),
                    if (p != null) ...[
...
class _FoodExpiryItemsScreen extends StatefulWidget {
  final Future<void> Function(BuildContext, {FoodExpiryItem? existing})? onUpsert;
...
class _FoodExpiryItemsScreenState extends State<_FoodExpiryItemsScreen> {
  ...
  Future<void> _addToCart(BuildContext context, FoodExpiryItem item) async {
    ...
    if (accountName == null) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('계정 정보를 불러올 수 없습니다.')),
        );
      }
      return;
    }
  ...
  Future<void> _applyBulkUsage() async {
    ...
    for (var entry in _usageMap.entries) {
      ...
      final item = items.firstWhere((i) => i.id == entry.key, orElse: () => items.first);
  ...
  Future<void> _showRecipePicker() async {
    ...
    if (selectedRecipe != null) {
      setState(() {
        ...
        for (var ingredient in selectedRecipe.ingredients) {
          ...
          try {
            final item = items.firstWhere(
              (i) => i.name.contains(ingredient.name) || ingredient.name.contains(i.name),
            );
        ...
        if (matchedCount > 0) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${selectedRecipe.name} 재료 $matchedCount개가 입력되었습니다.')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        builder: (context, items, child) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final leftText = left < 0 ? '지남 ${-left}일' : '남음 $left' '일';
              final color = left < 0
                ...
                title: Row(
                  children: [
                    ...
                    if (!_isUsageMode) ...[
                      ...
                      Padding(
                        ...
                        child: InkWell(
                          ...
                          child: Padding(
                            ...
                            child: Text(
                              '${it.quantity == it.quantity.toInt() ? it.quantity.toInt() : it.quantity}${it.unit}',
                      ...
                      Text(
                        '현재: ${it.quantity == it.quantity.toInt() ? it.quantity.toInt() : it.quantity}${it.unit}',
