...
class _ShoppingPointsInputScreenState extends State<ShoppingPointsInputScreen> {
  ...
  Future<void> _openDraftEditor(ShoppingPointsDraftEntry draft) async {
    ...
    final storeController = TextEditingController(text: (draft.store ?? '').trim());
    final cardController = TextEditingController(text: (draft.card ?? '').trim());
    final memoController = TextEditingController(text: (draft.memo ?? '').trim());
      ...
      builder: (dialogContext) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            final martDiscount = _parseMoneyOrZero(martDiscountController.text);
            final cardDiscount = _parseMoneyOrZero(cardDiscountController.text);
            final pointsDirect = _parseMoneyOrZero(pointsDirectController.text);
    ...
    if (result.total <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('구입 총액을 입력해 주세요.')),
      );
      return;
    }
    ...
    if (result.pointsDirect <= 0 && result.charged <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('카드결제금액을 입력해 주세요.')),
      );
      return;
    }
    ...
    if (result.charged > 0 && result.charged > result.total) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('카드결제금액이 합계금액보다 큽니다.')),
      );
      return;
    }
    ...
    final points = result.pointsDirect > 0 ? result.pointsDirect : computedPoints;
    ...
    if (points > 0) {
      final memoParts = <String>[
        ...
        if (result.charged > 0) '카드결제:${CurrencyFormatter.format(result.charged)}',
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(points > 0 ? '포인트를 저장했어요.' : '포인트 0원으로 처리했어요.'),
        ),
      );
    }
