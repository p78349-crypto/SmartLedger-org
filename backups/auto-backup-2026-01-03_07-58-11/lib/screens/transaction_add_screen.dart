...
// import 'package:smart_ledger/screens/nutrition_report_screen.dart'; // preserved but disabled per request
...
class _TransactionAddFormState extends State<TransactionAddForm> {
  ...
  void initState() {
    ...
    _paymentFocusNode.addListener(() {
      if (_paymentFocusNode.hasFocus) {
        ...
        _paymentController.selection = TextSelection(baseOffset: 0, extentOffset: text.length);
      }
    });
    _memoFocusNode.addListener(() {
      if (_memoFocusNode.hasFocus) {
        final text = _memoController.text;
        _memoController.selection = TextSelection(baseOffset: 0, extentOffset: text.length);
      }
    });
  ...
  Future<void> _loadRecentInputs() async {
    ...
    setState(() {
      _recentDescriptions =
          descriptions.take(_maxRecentDescriptions).toList(growable: false);
      _recentPayments = payments.take(_maxRecentPayments).toList(growable: false);
  ...
  Future<void> _saveTransaction() async {
    ...
    if (payment.isNotEmpty) {
      ...
      await prefs.setStringList(_recentPaymentsKey, updated.take(_maxRecentPayments).toList());
      _recentPayments = updated.take(_maxRecentPayments).toList();
    }
    if (memo.isNotEmpty) {
      final updated = [memo, ..._recentMemos.where((e) => e != memo)];
      await prefs.setStringList(_recentMemosKey, updated.take(_maxRecentMemos).toList());
      ...
      body: Form(
        ...
        child: ListView(
          padding: EdgeInsets.only(
            bottom: MediaQuery.of(context).padding.bottom + 80, // Add padding for FAB
          ),
      ...
      child: TextFormField(
        ...
        decoration: _standardInputDecoration(
          ...
          suffixIcon: IconButton(
            tooltip: '입력내용 불러오기',
                icon: Icon(Icons.list_alt, size: 20, color: Theme.of(context).iconTheme.color),
            onPressed: () => _showRecentInputPicker(
              context: context,
              items: _recentMemos,
              onSelected: (v) {
                _memoController.text = v;
                _memoController.selection = TextSelection.fromPosition(TextPosition(offset: v.length));
                setState(() {});
              },
              title: '메모 입력내용 불러오기',
            ),
      ...
      decoration: _standardInputDecoration(
        ...
        suffixIcon: IconButton(
          ...
          onPressed: () => _showRecentInputPicker(
            ...
            onSelected: (v) {
              controller.text = v;
              controller.selection = TextSelection.fromPosition(TextPosition(offset: v.length));
              setState(() {});
            },
  ...
  Widget _buildCategorySection() {
    ...
    final subCategories = categoryMap[_selectedMainCategory] ?? const <String>[];
          ...
          suffixIcon: IconButton(
            ...
            onPressed: () => _showRecentInputPicker(
              ...
              onSelected: (v) {
                ...
                _descController.selection = TextSelection.fromPosition(TextPosition(offset: v.length));
                setState(() {});
              },
              title: '상품명 입력내용 불러오기',
            ),
