...
class TransactionDbStore {
  ...
  Future<int?> ensureAccountId(String accountName) async {
    ...
    await _db.insertAccount(
      DbAccountsCompanion.insert(name: trimmed),
    );
  ...
  Future<void> upsertTransaction(
    ...
    final weatherJson = tx.weather == null ? null : jsonEncode(tx.weather!.toJson());
    ...
    final companion = DbTransactionsCompanion.insert(
      ...
      benefitJson: Value(tx.benefitJson?.trim().isEmpty ?? true ? null : tx.benefitJson!.trim()),
    );
  ...
  Future<void> upsertMany(
    ...
    DbTransactionsCompanion mapTx(model.Transaction tx) {
      final weatherJson =
          tx.weather == null ? null : jsonEncode(tx.weather!.toJson());
  ...
  Future<void> deleteTransaction(String accountName, String transactionId) async {
    final accountId = await ensureAccountId(accountName);
    if (accountId == null) return;

    await (_db.delete(_db.dbTransactions)
          ..where((tbl) => tbl.accountId.equals(accountId) & tbl.id.equals(transactionId)))
        .go();
  }
  ...
  Future<List<model.Transaction>> getAllTransactionsForAccount(
    ...
    final rows = await (_db.select(_db.dbTransactions)
