...
class _AssetDashboardScreenState extends State<AssetDashboardScreen> {
  ...
  Future<void> _loadProject100mPrefs() async {
    ...
    _projectYears = (prefs.getInt(PrefKeys.project100mYearsV1) ?? 10).clamp(
      1,
      50,
    );
    ...
    _projectSafeRatePct =
        (prefs.getDouble(PrefKeys.project100mSafeRatePctV1) ?? 3).clamp(
          0.0,
          100.0,
        );
          ...
          child: Column(
            ...
            children: [
              ...
              if (!achieved) ...[
                const SizedBox(height: 6),
                Text(
                  '추가로 매달 $extraMonthlyLabel 더 모으면 '
                  '목표에 가까워집니다.',
                  style: theme.textTheme.bodyMedium?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant,
                  ),
              ...
              if (_projectIncludeBenefits) ...[
                ...
                const SizedBox(height: 4),
                Text(
                  '팁: 작은 할인/포인트는 비상금(현금)에 모아두고, '
                  '일정 금액이 되면 투자로 전환하면 성장에 '
                  '도움이 됩니다.',
                  style: theme.textTheme.bodySmall?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant,
                  ),
