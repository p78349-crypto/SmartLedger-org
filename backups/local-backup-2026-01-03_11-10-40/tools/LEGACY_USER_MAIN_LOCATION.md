...
> UPDATE (2026-01-03): 현재 `AccountMainScreen`은 **One UI 아이콘 그리드 + 페이지 인디케이터 + 화면 보호기** 조합으로 고정되어 있습니다. 과거 Samsung QuickActions 전용 위젯은 삭제되었으며, 이 문서는 숨김 처리된 HomeTab 경로와 레거시 QuickActions 구조를 찾아볼 때 참고용으로만 유지합니다.
...
- `AccountMainScreen`은 **One UI 스타일 아이콘 페이지(최대 15페이지) + 배경/화면보호기 설정 + 사용자 편집 모드**로 구성되어 계정별 대시보드를 대체합니다.
...
- `lib/screens/account_main_screen.dart`에서 One UI 아이콘 그리드 대신 `HomeTabScreen(...)` 반환

(단, 현재 설계는 One UI 메인 고정이 목표이므로 기본은 비활성 유지)
