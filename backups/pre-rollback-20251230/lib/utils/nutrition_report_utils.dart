...
class NutritionReportUtils {
  ...
  static final RegExp _inlinePrice = RegExp(
    r'([가-힣A-Za-z]{1,20})\s*(?:\d+\s*(?:마리|개|봉|팩|g|kg|ml|l|L|리터)\s*)*(\d{2,6})(?:\s*(?:-|~)\s*(\d{2,6}))?\s*원',
  );
  ...
  static bool _looksLikeCola2L(String text) {
    ...
    final hasCola = lower.contains('콜라') || lower.contains('coke') || lower.contains('cola');
    if (!hasCola) return false;
    return lower.contains('2l') || lower.contains('2 l') || lower.contains('2리터') || lower.contains('2 리터');
  }
