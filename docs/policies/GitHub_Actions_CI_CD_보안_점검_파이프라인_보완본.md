# 🛡️ GitHub Actions CI/CD 보안 점검 파이프라인 (보완본)

이 문서는 프로젝트의 보안 점검을 자동화하는 GitHub Actions 파이프라인 구성과 로컬 환경에서의 실행 방법을 안내합니다. 기존 파이프라인에 **Gitleaks, Semgrep 통합 및 테스트 커버리지 리포트 저장 기능**이 보완되었습니다.
...
- **비밀키 및 정적 분석 (SAST)**: `gitleaks`, `semgrep`
  ...
  security-check:
    ...
    steps:
      ...
      - name: Run Gitleaks
        ...
        with:
          report-path: security_reports/gitleaks_report.json

      # 7. Semgrep (정적 분석)
      - name: Run Semgrep
        run: |
          pip install semgrep
          semgrep --config=p/dart --sarif --output=security_reports/semgrep_report.json .
      ...
      # 11. 정적/동적 보안 스캔 (주의: 별도 서버 세팅 필요)
      - name: SonarQube Scan
        run: sonar-scanner
        continue-on-error: true # 서버가 없을 경우를 대비해 에러 무시

      - name: OWASP ZAP Scan
        run: zap-cli quick-scan http://localhost:8080
        continue-on-error: true # 서버가 없을 경우를 대비해 에러 무시

      # 12. 보안 점검 결과 및 커버리지 아티팩트 업로드
      - name: Upload Security Reports and Coverage
        uses: actions/upload-artifact@v4
        if: always() # 실패하더라도 리포트는 업로드
        with:
          name: security-and-coverage-reports
          path: |
            security_reports/
            coverage/
```
...
4. **CI/CD 파이프라인 동작 확인**: GitHub에 코드를 Push하거나 PR을 생성하여 Actions 탭에서 파이프라인이 정상적으로 트리거되고, Artifacts에 `security-and-coverage-reports`가 업로드되는지 확인합니다.
