...
extracted_codes = []
# Keep tracking offsets or delete from bottom up. Let's just find bounds, sort by end_line descending, and delete.
bounds = []
for m in ui_methods:
...
if bounds:
    ...
    if last_brace_idx != -1:
        # replace the last brace with the codes and a new brace
        ui_lines[last_brace_idx] = '\n\n' + '\n\n'.join(extracted_codes) + '\n}\n'
        with open(ui_path, 'w', encoding='utf-8') as f:
            f.writelines(ui_lines)
            
    print('UI methods extracted successfully.')
else:
