...
def extract_methods():
    source_file = 'lib/screens/wms_pda_quick_input_screen.dart'
    logic_file = 'lib/screens/wms_pda_quick_input_screen_logic.dart'
    ...
    if "part 'wms_pda_quick_input_screen_logic.dart';" not in joined_lines:
        imports_end = len(new_lines) - 1
        for i, line in enumerate(new_lines):
            if line.startswith('class '):
                imports_end = i
                break
        new_lines.insert(imports_end, "part 'wms_pda_quick_input_screen_logic.dart';\n\n")

    with open(source_file, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
        
    joined_extractions = "\n".join(extractions)
    logic_content = f"""part of 'wms_pda_quick_input_screen.dart';
