...
class DbTransactions extends Table {
  ...
  TextColumn get mainCategory =>
      text().withDefault(const Constant('미분류'))();
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (migrator, from, to) async {
      ...
      if (from < 4) {
        await migrator.addColumn(dbTransactions, dbTransactions.cardChargedAmount);
        ...
        await migrator.addColumn(dbTransactions, dbTransactions.savingsAllocation);
        await migrator.addColumn(dbTransactions, dbTransactions.isRefund);
        await migrator.addColumn(dbTransactions, dbTransactions.originalTransactionId);
  ...
  Future<DbAccount?> getAccountByName(String name) {
    return (select(dbAccounts)..where((tbl) => tbl.name.equals(name)))
        .getSingleOrNull();
  }
}
