...
extension AccountStatsStoreProducts on _AccountStatsScreenState {
  Widget _buildStoreProductsInline(
    ThemeData theme, List<Transaction> txs,
  ) {
    final store = _defaultStore;
    if (store == null || store.trim().isEmpty) {
      return Card(child: Padding(
        padding: const EdgeInsets.all(16),
        child: Text('마트/쇼핑몰명이 기록된 거래가 없습니다.',
    ...
    for (final t in txs) {
      ...
      final acc = byKey.putIfAbsent(
          key, () => StoreProductAcc(name: t.description.trim()));
      acc.count += 1;
      acc.total += t.amount;
    ...
    final items = byKey.values
        .map((a) => StoreProductStat(
            name: a.name, count: a.count, total: a.total))
          ...
          children: [
            Row(children: [
              ...
              Expanded(child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('기본 마트/쇼핑몰: $store',
                    style: theme.textTheme.labelMedium?.copyWith(
                        color: theme.colorScheme.onSurfaceVariant)),
                  Text(_formatWon(thisMonthTotal),
              ...
              ...top.asMap().entries.map((entry) {
                ...
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(children: [
                        Expanded(child: Text('$rank. ${item.name}',
                          style: theme.textTheme.bodyMedium?.copyWith(
                              fontWeight:
                                  rank <= 20 ? FontWeight.w600 : FontWeight.w400),
                          overflow: TextOverflow.ellipsis)),
                        const SizedBox(width: 8),
                        Text(_formatWon(item.total),
                          style: theme.textTheme.bodySmall?.copyWith(
                              color: theme.colorScheme.onSurfaceVariant,
