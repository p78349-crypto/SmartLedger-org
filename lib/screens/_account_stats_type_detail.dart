...
extension AccountStatsTypeDetail on _AccountStatsScreenState {
  Widget _buildTypeDetailView(
    ...
    final filtered = transactions.where((tx) {
      ...
      if (_selectedDate != null) {
        inRange = tx.date.year == _selectedDate!.year &&
            tx.date.month == _selectedDate!.month &&
            tx.date.day == _selectedDate!.day;
      } else {
        inRange = tx.date.isAfter(
    ...
    for (final tx in filtered) {
      ...
      categoryTotals[cat] =
          (categoryTotals[cat] ?? 0) + tx.amount.abs();
      categoryTransactions.putIfAbsent(cat, () => []).add(tx);
    }
    final sortedCategories = categoryTotals.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    final total =
        filtered.fold<double>(0, (sum, tx) => sum + tx.amount.abs());
      ...
      children: [
        Row(children: [
          IconButton(
            ...
            onPressed: () =>
                setState(() => _selectedView = StatsView.month),
          ),
          Text('${_typeLabel(type)} 상세',
        ...
        _buildTypeDetailTotalCard(
            theme, type, total, filtered, endOfMonth),
        const SizedBox(height: 24),
        ..._buildTypeDetailCategoryList(
            sortedCategories, categoryTransactions,
            total, type, theme),
      ],
    );
  }
  ...
  Widget _buildTypeDetailMonthNav(ThemeData theme) {
    return Row(
      ...
      children: [
        ...
        Text(_monthLabelFormat.format(_currentMonth),
  ...
  Widget _buildTypeDetailDatePicker(
      ThemeData theme, TransactionType type) {
    return Center(
      child: Card(
        elevation: 0,
          ...
          if (filtered.isNotEmpty && _selectedDate == null) ...[
            ...
            Text('일일 평균',
                style: theme.textTheme.bodyMedium?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant)),
            const SizedBox(height: 4),
            Text(_formatDailyAverage(total, endOfMonth.day),
