          ...
          child: Column(
            ...
            children: [
              Row(children: [
                ...
                Expanded(child: Text('${report.currentWeek}주차 주간 리포트',
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: burnColor.withValues(alpha: 0.3)),
                ),
                child: Row(children: [
                  Expanded(child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('소진 속도 (Burn Rate)',
                          style: theme.textTheme.labelMedium),
                      const SizedBox(height: 4),
                      Text('${(report.burnRate * 100).toInt()}%',
                        style: theme.textTheme.displaySmall?.copyWith(
                            color: burnColor, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Text(report.burnRate > 1.0
                          ? '예산보다 빨리 쓰고 있어요' : '안정적인 페이스입니다',
                        style: theme.textTheme.bodySmall?.copyWith(
                            color: burnColor)),
                    ],
                  )),
                  Icon(report.burnRate > 1.0
                      ? Icons.local_fire_department_rounded
                      : Icons.check_circle_outline_rounded,
                    size: 48, color: burnColor),
                ]),
              ),
              const SizedBox(height: 24),
              Text(report.message, style: theme.textTheme.bodyLarge?.copyWith(
                  fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text(report.subMessage, style: theme.textTheme.bodyMedium
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    _buildSimpleStatItem(theme, '주간 표준 예산',
                        _currencyFormat.format(report.weeklyBudget)),
                    Container(
                        width: 1, height: 40,
                        color: scheme.outlineVariant),
                    _buildSimpleStatItem(theme, '이번 주 권장 한도',
