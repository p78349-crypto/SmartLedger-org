  ...
  Widget _buildYearView(List<Transaction> transactions, ThemeData theme) {
    ...
    final summary = canUseAgg
        ? () {
            ...
            final fixedMonthly =
                _fixedCostTotalForMonth(DateTime(_currentYear));
            final fixedYearly =
                _fixedCosts.isEmpty ? 0.0 : fixedMonthly * 12;
    ...
    final monthSummaries = List.generate(12, (index) {
      ...
      if (canUseAgg) {
        ...
        final total =
            baseTotal + (addFixed ? fixedMonthlyTotal : 0.0);
      ...
      final yearlyTxs =
          transactions.where((tx) => tx.date.year == _currentYear);
      final monthTxs = yearlyTxs.where((tx) =>
