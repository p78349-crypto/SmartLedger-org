...
class _AccountStatsScreenState extends State<AccountStatsScreen> {
  ...
  List<QuickSimpleExpenseInputEntry> _quickEntries = const <QuickSimpleExpenseInputEntry>[];
  ...
  String? _pickDefaultStore(
    ...
    final ordered = List<Transaction>.from(txs)..sort((a, b) => b.date.compareTo(a.date));
  ...
  Widget _buildStoreProductsInline(ThemeData theme, List<Transaction> txs) {
    ...
    final items = byKey.values
        .map((a) => _StoreProductStat(name: a.name, count: a.count, total: a.total))
        .toList(growable: false)
  ...
  Widget _buildYearView(List<Transaction> transactions, ThemeData theme) {
    ...
    final summary = canUseAgg
        ? () {
            ...
            final fixedCostYearly =
                _fixedCosts.isEmpty ? 0.0 : fixedCostMonthly * 12;
    ...
    final monthSummaries = List.generate(12, (index) {
      ...
      if (canUseAgg) {
        ...
        final bucket = cache.months[ym];
        final baseTotal = bucket?.amountForType(_currentType) ?? 0.0;
        final addFixed = _includeFixedCosts &&
            _fixedCosts.isNotEmpty &&
            _currentType == TransactionType.expense;
...
_TxSearchPlan _parseTxSearchPlan(String raw) {
  ...
  void applyAmount(String op, double value) {
    ...
    switch (op) {
      case '>=':
      case '>':
        filters.minAmountAbs =
            (filters.minAmountAbs == null) ? v : (filters.minAmountAbs! > v ? filters.minAmountAbs : v);
        break;
      case '<=':
      case '<':
        filters.maxAmountAbs =
            (filters.maxAmountAbs == null) ? v : (filters.maxAmountAbs! < v ? filters.maxAmountAbs : v);
        break;
      case '=':
  ...
  void applyBenefitAmount(String op, double value) {
    ...
    switch (op) {
      ...
      case '>':
        filters.minBenefit =
            (filters.minBenefit == null) ? v : (filters.minBenefit! > v ? filters.minBenefit : v);
        break;
      case '<=':
      case '<':
        filters.maxBenefit =
            (filters.maxBenefit == null) ? v : (filters.maxBenefit! < v ? filters.maxBenefit : v);
        break;
      case '=':
...
bool _matchesTxFilters(Transaction tx, _TxSearchFilters f) {
  ...
  if (f.memoContains.isNotEmpty && !_matchesAllContains(tx.memo, f.memoContains)) {
    return false;
  }

  if (f.benefitOnly || f.pointsOnly || f.minBenefit != null || f.maxBenefit != null) {
    ...
    if (f.pointsOnly) {
      final hasPoints = byType.keys.any((k) {
        ...
        return key.contains('포인트') || key.contains('적립') || key.contains('point');
      });
...
class _AccountStatsSearchScreenState extends State<AccountStatsSearchScreen> {
  ...
  void _runSearch(TransactionService service, String rawQuery) {
    ...
    () async {
      ...
      if (plan.ftsQuery.trim().isNotEmpty) {
        ...
        // Filter-only query (e.g., ">=10000", "카드:현대")
  ...
  void _maybeLoadPointProjection(int seq, _TxSearchPlan plan) {
    ...
    () async {
      ...
      final fiveYear =
          _futureValueMonthlyContribution(monthlyBase, rate, 12 * 5);
      final tenYear =
          _futureValueMonthlyContribution(monthlyBase, rate, 12 * 10);

        final fiveYear3 =
          _futureValueMonthlyContribution(threeMonthAvg, rate, 12 * 5);
        final tenYear3 =
          _futureValueMonthlyContribution(threeMonthAvg, rate, 12 * 10);

          final fiveYear6 =
            _futureValueMonthlyContribution(sixMonthAvg, rate, 12 * 5);
          final tenYear6 =
            _futureValueMonthlyContribution(sixMonthAvg, rate, 12 * 10);
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  '평균: ${_currencyFormat.format(avg)}원',
                  style: subStyle,
                ),
              ],
            ),
            ...
            if (f.pointsOnly) ...[
              const SizedBox(height: 6),
              Text(
                _pointProjectionLoading
                  ? '이번달(최우선) 포인트 기반 5/10년 예상(연 '
                        '${_pointProjectionAnnualRateUsed.toStringAsFixed(0)}%): '
                        '계산 중…'
                    : (_pointProjectionMonthlyBase == null ||
                            (_pointProjectionMonthlyBase ?? 0) <= 0)
                    ? '이번달(최우선) 포인트 기반 5/10년 예상(연 '
                            '${_pointProjectionAnnualRateUsed.toStringAsFixed(0)}%): '
                            '0원'
                    : '이번달(최우선) 포인트 ${_currencyFormat.format(_pointProjectionMonthlyBase ?? 0)}원'
                            ' → 5년(연 ${_pointProjectionAnnualRateUsed.toStringAsFixed(0)}%): '
                            '${_currencyFormat.format(_pointProjectionFiveYear ?? 0)}원'
                            ' · 10년: ${_currencyFormat.format(_pointProjectionTenYear ?? 0)}원',
              ...
              if (!_pointProjectionLoading &&
                ...
                const SizedBox(height: 4),
                Text(
                  '참고(과거) 최근 3개월 평균 ${_currencyFormat.format(_pointProjectionMonthlyBase3mAvg ?? 0)}원'
                  ' → 5년: ${_currencyFormat.format(_pointProjectionFiveYear3mAvg ?? 0)}원'
                  ' · 10년: ${_currencyFormat.format(_pointProjectionTenYear3mAvg ?? 0)}원',
                  style: readableSubStyle,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
              ...
              if (!_pointProjectionLoading &&
                ...
                const SizedBox(height: 4),
                Text(
                  '참고(과거) 최근 6개월 평균 ${_currencyFormat.format(_pointProjectionMonthlyBase6mAvg ?? 0)}원'
                  ' → 5년: ${_currencyFormat.format(_pointProjectionFiveYear6mAvg ?? 0)}원'
                  ' · 10년: ${_currencyFormat.format(_pointProjectionTenYear6mAvg ?? 0)}원',
                  style: readableSubStyle,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                ),
