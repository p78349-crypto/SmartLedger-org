...
class DataFlexibleExportScreen extends StatefulWidget {
  ...
  @override
  State<DataFlexibleExportScreen> createState() => _DataFlexibleExportScreenState();
}
...
class _DataFlexibleExportScreenState extends State<DataFlexibleExportScreen> {
  ...
  void _applyPreset(String preset) {
    setState(() {
      switch (preset) {
        case 'full':
          _colType = _colDate = _colCategory = _colTitle = _colAmount = _colPayment = _colMemo = true;
    ...
    try {
      ...
      final List<Asset> allAssets = _includeAssets
          ? (_selectedAccount == 'ROOT'
              ? _allAccounts.where((n) => n != 'ROOT').expand((n) => AssetService().getAssets(n)).toList()
      ...
      if (!mounted) return;
      if (resultFile != null) {
        SnackbarUtils.showSuccess(context, '${_useExcel ? '엑셀' : 'CSV'} 파일 저장 완료: ${resultFile.path}');
      } else {
          ...
          children: [
            ...
            Card(
              child: ListTile(
                ...
                title: Text(_selectedAccount == 'ROOT' ? '전체 계정 (ROOT)' : _selectedAccount),
              ...
              child: Padding(
                ...
                child: Wrap(
                  children: [
                    ...
                    _buildFilterChip('카테고리', _colCategory, (v) => _colCategory = v),
                    _buildFilterChip('항목명', _colTitle, (v) => _colTitle = v),
                    _buildFilterChip('금액', _colAmount, (v) => _colAmount = v),
                    _buildFilterChip('지불/보관기관', _colPayment, (v) => _colPayment = v),
            ...
            Card(
              color: _excludePersonalInfo ? scheme.secondaryContainer.withValues(alpha: 0.3) : null,
              child: SwitchListTile(
                title: const Text('타인 제공용 (개인정보 마스킹)'),
                subtitle: const Text('장소명, 기관명, 메모 등을 별표(*) 처리합니다'),
                secondary: Icon(
                  _excludePersonalInfo ? Icons.privacy_tip : Icons.privacy_tip_outlined,
  ...
  void _showAccountPicker() {
    showModalBottomSheet(
      ...
      builder: (context) => ListView(
        shrinkWrap: true,
        children: _allAccounts.map((name) => ListTile(
          title: Text(name),
          onTap: () {
