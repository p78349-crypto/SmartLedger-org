...
class _FoodSearchResultState extends State<_FoodSearchResult> {
  ...
  void didUpdateWidget(covariant _FoodSearchResult oldWidget) {
    ...
    if (oldWidget.query != widget.query) _selectedIndex = 0;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (trimmed.isEmpty) {
      return Padding(
        ...
        child: Center(child: Text('식재료를 입력하세요. 예: 닭고기',
    ...
    if (entry == null) {
      return Padding(
        padding: const EdgeInsets.symmetric(vertical: 24),
        child: Center(child: Text('"$trimmed" 데이터가 없습니다.\n(예: 닭고기, 계란, 두부)',
  ...
  Widget _buildBody(BuildContext context, FoodKnowledgeEntry entry) {
    switch (_selectedIndex) {
      case 0: return _buildIntakeInfo(context, entry);
      case 1: return _buildPairings(context, entry);
      case 2: return _buildQuantities(context, entry);
