...
class _NutritionHighlights extends StatelessWidget {
  ...
  bool _has(String key) => items.any((e) => e.name.contains(key));
  ...
  Widget build(BuildContext context) {
    ...
    if (bullets.isEmpty) {
      return Text('메모에 식재료 이름을 포함하면 포인트가 더 정확해져요.',
...
class _DessertSuggestions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('식사 후 조금 부족한 영양을 보충하는 간단한 조합',
          style: theme.textTheme.bodySmall?.copyWith(
