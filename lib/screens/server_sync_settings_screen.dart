...
class _ServerSyncSettingsScreenState extends State<ServerSyncSettingsScreen> {
              ...
              onPressed: () async {
                ...
                if (result.isSuccess) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        result.message ?? '복구키 기반 복구에 실패했습니다.',
                      ),
  ...
  Future<void> _showServerConfigDialog() async {
    ...
    final currentAllowSelfSigned =
        await serverConfigService.getAllowSelfSignedCert();
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    if (address.isEmpty || key.isEmpty) {
                      ...
                      ScaffoldMessenger.of(dialogContext).showSnackBar(
                        const SnackBar(
                          content: Text('서버 주소와 관리자 키를 입력하세요'),
                        ),
                      );
                      return;
                    }
