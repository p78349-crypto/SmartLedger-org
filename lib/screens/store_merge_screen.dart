...
class _StoreMergeScreenState extends State<StoreMergeScreen> {
  ...
  Future<void> _editAlias(_StoreCount item) async {
            ...
            if (currentCanonical != null)
              TextButton(
                onPressed: () {
                  Navigator.of(
                    dialogContext,
                  ).pop(const _StoreMergeDialogResult(remove: true));
                },
                child: const Text('해제'),
              ),
            FilledButton(
              onPressed: () {
                Navigator.of(dialogContext).pop(
                  _StoreMergeDialogResult(canonical: controller.text.trim()),
                );
              },
              child: const Text('저장'),
            ),
    ...
    if (result.remove) {
      await StoreAliasService.removeAlias(widget.accountName, alias: alias);
    } else {
    ...
    final message = canonNow == alias ? '병합 해제됨' : '대표 마트명: $canonNow';
                ...
                children: [
                  ...
                  if (_stores.isEmpty)
                    ...
                    ..._stores.map((item) {
                      ...
                      final mapped =
                          _aliasMap.containsKey(item.store) &&
