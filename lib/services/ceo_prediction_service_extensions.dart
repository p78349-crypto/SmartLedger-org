...
extension CeoPredictionServiceExtensions on CeoPredictionService {
  ...
  List<String> _identifyKeyFactors(List<Transaction> transactions, List<Asset> assets) {
  ...
  Map<String, double> _calculateWeeklyTrends(List<CeoPredictionModel> predictions) {
  ...
  List<String> _generateRecommendations(
    ...
    final avgRisk = predictions.map((p) => p.riskScore.overall).reduce((a, b) => a + b) / predictions.length;
    
    if (trends['weekly_total']! < 0) {
      recommendations.add('Monitor cash flow - projected weekly deficit detected');
    }
    
    if (avgRisk > 0.7) {
      recommendations.add('High risk detected - consider portfolio rebalancing');
    }
    
    if (trends['avg_confidence']! < 0.7) {
      recommendations.add('Low prediction confidence - gather more financial data');
    }
