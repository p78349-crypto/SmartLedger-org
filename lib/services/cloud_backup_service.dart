...
class CloudBackupService {
  ...
  Future<CloudBackupStatus> performCloudBackup({
    ...
    try {
      ...
      if (!localBackupResult['success']) {
        return _createFailedStatus(
          backupId,
          'Local backup failed: ${localBackupResult['error']}',
        );
      }
      ...
      final uploadResult = await _uploadToCloud(
        ...
        customFileName ??
            'smartledger_${DateTime.now().millisecondsSinceEpoch}.slb',
        backupId,
      );
  ...
  Future<void> _scheduleAutomaticBackups(
    ...
    _autoBackupTimer = Timer.periodic(interval, (_) async {
      ...
      if (CloudBackupHelper.shouldRunAutomaticBackup(
        config,
        lastBackup,
        lastStatus,
      )) {
  ...
  Stream<CloudBackupStatus>? get backupStatusStream =>
