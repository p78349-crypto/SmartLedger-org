...
extension FinancialAnalyticsServiceExtensions on FinancialAnalyticsService {
  ...
  Map<String, dynamic> _analyzeInflowPattern(List<Transaction> transactions) {
    ...
    if (inflowTransactions.isEmpty) {
      return {
        'dailyAverage': 0.0,
        'weeklyPattern': <double>[],
        'monthlyPattern': <double>[],
      };
    }
    ...
    for (final transaction in inflowTransactions) {
      ...
      dailyTotals[dayOfWeek] =
          (dailyTotals[dayOfWeek] ?? 0.0) + transaction.amount;
      monthlyTotals[dayOfMonth] =
          (monthlyTotals[dayOfMonth] ?? 0.0) + transaction.amount;
    }

    final totalInflow = inflowTransactions.fold(
      0.0,
      (sum, t) => sum + t.amount,
    );
  ...
  Map<String, dynamic> _analyzeOutflowPattern(List<Transaction> transactions) {
    final outflowTransactions = transactions
        .where((t) => t.amount < 0)
        .toList();
    ...
    for (final transaction in outflowTransactions) {
      ...
      monthlyTotals[dayOfMonth] =
          (monthlyTotals[dayOfMonth] ?? 0.0) + absAmount;
    }

    final totalOutflow = outflowTransactions.fold(
      0.0,
      (sum, t) => sum + t.amount.abs(),
    );
    final avgDaily =
        totalOutflow / max(1, _getDaysSpanned(outflowTransactions));
