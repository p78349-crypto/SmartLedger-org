...
class ProductDataImporter {
  ...
  Future<ImportResult> importKoreanProductsFromCsv(
    ...
    try {
      AppLogger.info(
        '[Importer] Starting Korean product import from: $filePath',
      );
      ...
      for (int i = 0; i < products.length; i += batchSize) {
        ...
        try {
          ...
          AppLogger.info(
            '[Importer] Batch inserted: ${batch.length} (total: ${result.inserted})',
          );
        } catch (e) {
      ...
      AppLogger.info(
        '[Importer] ✓ Import complete! Korean products: $finalCount',
      );
  ...
  Map<String, dynamic>? _parseCsvLine(String line, String timestamp) {
    try {
      ...
      if (parts.length < 5) {
        AppLogger.warn(
          '[Parser] Invalid format (${parts.length} columns, expected 5+)',
        );
        return null;
      }
