...
class TransactionDbMigrationService {
  ...
  Future<TransactionDbMigrationResult> ensureMigratedFromPrefs({
    ...
    if (already) {
      return const TransactionDbMigrationResult(performed: false, totalImported: 0);
    }
    ...
    try {
      ...
      // If legacy JSON is corrupted, do not block app; mark migrated to avoid loops.
      await _markMigrated(prefs);
      return const TransactionDbMigrationResult(performed: false, totalImported: 0);
    }
    ...
    for (final entry in decoded.entries) {
      ...
      // (If the app previously had DB transactions for this account, upsert will
      //  keep count potentially larger. That's fine.)
      ...
      if (countAfter < txs.length) {
        ...
        return TransactionDbMigrationResult(performed: true, totalImported: totalImported);
      }
