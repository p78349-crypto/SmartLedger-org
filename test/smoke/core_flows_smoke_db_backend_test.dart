...
void main() {
  ...
  testWidgets('SMOKE(DB+AUTH): RootAuthGate bypass opens child in DB mode', (
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: RootAuthGate(
          child: Scaffold(body: Text('ROOT_DB_OPEN')),
        ),
      ),
    );
  ...
  testWidgets('SMOKE(DB+AUTH): AssetRouteAuthGate bypass opens child in DB mode', (
    tester,
  ) async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      PrefKeys.txStorageBackendV1: 'db',
  ...
  testWidgets('SMOKE(DB): Transaction save reflects in AccountStatsScreen summary', (
    tester,
  ) async {
    SharedPreferences.setMockInitialValues(<String, Object>{
      PrefKeys.txStorageBackendV1: 'db',
    ...
    final accountName = 'smoke_db_stats_${DateTime.now().microsecondsSinceEpoch}';
    ...
    Future<void> enterTextField(String key, String value) async {
      ...
      final textField = find.descendant(of: field, matching: find.byType(TextFormField));
